...
y genera un Excel consolidado por persona.
  ...
  PAGO TOTAL SS EMPRESA (estructurado JSON - formato colombiano $ X.XXX):
  K - PENSIÓN emp.     <- "Unnamed: 31"                  (AF)
  L - FSP emp.         <- "Unnamed: 32" + "Unnamed: 33"  (AG + AH)
  M - SALUD emp.       <- "Unnamed: 41"                  (AP)
  N - ARL              <- "Unnamed: 48"                  (AW)
  O - CCF              <- "Unnamed: 52"                  (BA)
  P - SENA             <- "SENA"                         (BB)
  Q - ICBF             <- "ICBF"                         (BC)
...
def extraer_est(registros: list) -> dict:
    """Suma los valores de empresa a través de TODOS los registros del estructurado."""
    totales = {"pension_emp": 0.0, "fsp_emp": 0.0, "salud_emp": 0.0,
               "arl": 0.0, "ccf": 0.0, "sena": 0.0, "icbf": 0.0}
    for r in registros:
        totales["pension_emp"] += parse_valor_col(r.get("Unnamed: 31", "0"))
        ag = parse_valor_col(r.get("Unnamed: 32", "0"))
        ah = parse_valor_col(r.get("Unnamed: 33", "0"))
        totales["fsp_emp"]   += ag + ah
        totales["salud_emp"] += parse_valor_col(r.get("Unnamed: 41", "0"))
        totales["arl"]       += parse_valor_col(r.get("Unnamed: 48", "0"))
        totales["ccf"]       += parse_valor_col(r.get("Unnamed: 52", "0"))
        totales["sena"]      += parse_valor_col(r.get("SENA",        "0"))
        totales["icbf"]      += parse_valor_col(r.get("ICBF",        "0"))
    return totales
...
def detectar_pares() -> list[tuple[str, Path, Path]]:
    """
    Busca pares  *_estructurado.json + *_facturas.json con el mismo prefijo.
    Devuelve lista de (prefijo, ruta_estructurado, ruta_facturas).
    """
    estructurados = {f.stem.replace("_estructurado", ""): f
                     for f in CARPETA_DATOS.glob("*_estructurado.json")}
    facturas = {f.stem.replace("_facturas", ""): f
                for f in CARPETA_DATOS.glob("*_facturas.json")}

    pares = []
    for prefijo in sorted(estructurados):
        if prefijo in facturas:
            pares.append((prefijo, estructurados[prefijo], facturas[prefijo]))
        else:
            print(f"  Aviso: '{prefijo}_estructurado.json' sin facturas — se omite.")
    return pares
...
def procesar_par(prefijo: str, ruta_est: Path, ruta_fac: Path):
    ...
    for doc in todos_docs:
        ...
        # A/B/C: del primer registro del estructurado; si no hay, de facturas
        primer_est = regs_est[0] if regs_est else {}
        primer_fac = reg_fac[0]  if reg_fac  else {}
        tipo   = str(primer_est.get("IDENTIFICA CIÓN - Tipo",   "")).strip() or "CC"
        numero = str(primer_est.get("IDENTIFICA CIÓN - Número", doc)).strip()
        nombre = (str(primer_est.get("NOMBRES", "")).strip()
