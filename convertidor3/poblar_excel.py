...
y genera un Excel consolidado por persona.
  ...
  K - PENSIÓN emp.     <- clave "24" - (clave "23" * 1%)  (AFP Aporte menos FSP)
  L - FSP emp.         <- clave "23" * 1%                  (1% del IBC AFP)
  M - SALUD emp.       <- clave "28"                       (EPS Aporte)
  N - ARL              <- clave "37"                       (ARL Aporte)
  O - CCF              <- clave "32"                       (CCF Aporte)
  P - SENA             <- clave "40" * 40%                 (40% Parafiscales)
  Q - ICBF             <- clave "40" * 60%                 (60% Parafiscales)
"""
...
def extraer_est(registros: list) -> dict:
    ...
    Claves numéricas: "23"=AFP IBC, "24"=AFP Aporte, "28"=EPS Aporte,
                      "32"=CCF Aporte, "37"=ARL Aporte, "40"=Para Aporte
    """
    totales = {"pension_emp": 0.0, "fsp_emp": 0.0, "salud_emp": 0.0,
               "arl": 0.0, "ccf": 0.0, "sena": 0.0, "icbf": 0.0}
    for r in registros:
        afp_ibc    = parse_planilla(r.get("23", "0"))
        afp_aporte = parse_planilla(r.get("24", "0"))
        totales["pension_emp"] += afp_aporte - (afp_ibc * 0.01)
        totales["fsp_emp"]     += afp_ibc * 0.01
        totales["salud_emp"]   += parse_planilla(r.get("28", "0"))
        totales["arl"]         += parse_planilla(r.get("37", "0"))
        totales["ccf"]         += parse_planilla(r.get("32", "0"))
        para = parse_planilla(r.get("40", "0"))
